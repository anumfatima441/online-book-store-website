...
$cart_query = mysqli_query($conn, "SELECT cart.id AS cart_id, cart.quantity, books.* FROM `cart` JOIN `books` ON cart.book_id = books.id WHERE cart.user_id = '$user_id'") or die('query failed: ' . mysqli_error($conn));
...
<div class="container mt-5">
                ...
                <tbody>
                    <?php while($cart_item = mysqli_fetch_assoc($cart_query)){
                        ...
                        if(empty($image) || strpos($image, 'via.placeholder.com') !== false){
                            $image = $book_covers[$cart_item['title']] ?? 'https://via.placeholder.com/300x300?text=No+Image';
                        }
                    ?>
                    <tr>
                        <td>
                        ...
                        <td><a href="cart.php?remove_id=<?php echo $cart_item['cart_id']; ?>" class="btn btn-danger btn-sm">Remove</a></td>
                    </tr>
                    <?php } ?>
                    <tr>
                        <td colspan="3" class="text-end fw-bold">Total</td>
                        <td colspan="2" class="fw-bold">Rs. <?php echo number_format($total_price, 2); ?></td>
                    </tr>
                </tbody>
            </table>
        </div>
    ...
    <?php } else { ?>
        <p class="mt-4">Your cart is empty. Add books from the homepage.</p>
    <?php } ?>
</div>
